        ...
        for j in 0..m {
            ...
            for (di, dj) in &dirs {
                ...
                if ni >= 0
                    ...
                    && nj >= 0
                    && nj < m as i32
                    && layout[ni as usize][nj as usize] == '#'
                {
...
fn part1(content: &str) {
    let mut layout: Layout = content.lines().map(|line| line.chars().collect()).collect();
    ...
    let part1 = layout.into_iter().flatten().filter(|c| *c == '#').count();
...
fn part2(content: &str) {
    ...
    let part2 = layout.into_iter().flatten().filter(|c| *c == '#').count();
