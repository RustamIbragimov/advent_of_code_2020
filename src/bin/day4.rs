...
impl<'a> Passport<'a> {
    ...
    fn validate_fields_format(&self) -> bool {
        if !is_n_digit_number(self.fields["byr"], 4)
            || !is_number_in_range(self.fields["byr"], 1920, 2002)
        {
            return false;
        }
        if !is_n_digit_number(self.fields["iyr"], 4)
            || !is_number_in_range(self.fields["iyr"], 2010, 2020)
        {
            return false;
        }
        if !is_n_digit_number(self.fields["eyr"], 4)
            || !is_number_in_range(self.fields["eyr"], 2020, 2030)
        {
            return false;
        }
...
fn is_valid_eye_color(color: &str) -> bool {
    let valid_colors: HashSet<_> = vec!["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
        .into_iter()
        .collect();
...
fn is_n_digit_number(s: &str, expected_digits_count: usize) -> bool {
    match s.parse::<i32>() {
        Ok(_) => s.len() == expected_digits_count,
        Err(_) => false,
    }
}
...
fn is_number_in_range(s: &str, from: i32, to: i32) -> bool {
    match s.parse::<i32>() {
        Ok(number) => number >= from && number <= to,
        Err(_) => false,
    }
}
    ...
    for line in content.lines() {
        if line.is_empty() {
            passports.push(Passport {
                fields: current_passport_fields,
            });
